...
export interface ArticlesDao {
  ...
  saveArticle(userId: IUser["user_id"], articleId: IArticle["article_id"]): Promise<void>;
  ...
  isArticleSaved(userId: IUser["user_id"], articleId: IArticle["article_id"]): Promise<boolean>;

  /* ===== Update Operations ===== */

  // Updates an existing article by ID
  updateArticle(articleId: IArticle["article_id"], article: Partial<IArticle>): Promise<IArticle>;

  /* ===== Delete Operations ===== */

  // Removes a saved article for the user.
  unsaveArticle(userId: IUser["user_id"], articleId: IArticle["article_id"]): Promise<void>;
