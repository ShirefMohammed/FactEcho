import { AuthorFields, IAuthor, IAuthorPermissions, UserFields } from "@/types/entitiesTypes";
...
export class AuthorsModel implements AuthorsDao {
  ...
  async findAuthorById(
    ...
    try {
      ...
      const authorUserData = await usersModel.findUserById(authorId, selectedFields);
      ...
      const permissionsQuery = `
        ...
        WHERE user_id = $1
      `;
      const { rows: permissionsRows } = await pool.query(permissionsQuery, [authorId]);
  ...
  async deleteAuthorPermissions(authorId: string): Promise<void> {
    try {
      await pool.query(`DELETE FROM author_permissions WHERE user_id = $1`, [authorId]);
    } catch (err) {
