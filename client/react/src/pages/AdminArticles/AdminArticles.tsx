...
const AdminArticles = () => {
  ...
  useEffect(() => {
    searchKey
      ? navigate(`/admin/articles?searchKey=${searchKey}&page=${searchArticlesPage}&limit=${limit}`)
      : navigate(`/admin/articles?page=${articlesPage}&limit=${limit}`);
...
const ArticleRow = ({
  ...
  const articlesAPIs = useArticlesAPIs(); // API abstraction for article-related actions
  ...
  return (
    <tr className="border-b border-slate-300 dark:border-slate-700 text-sm text-body bg-white dark:bg-boxdark dark:text-bodydark font-normal">
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        {article.image ? (
          <img className="w-6 h-6 rounded-sm" src={article.image} alt="article image" />
        ) : (
          "---"
        )}
      </td>
