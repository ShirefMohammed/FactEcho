    ...
    <footer className="bg-white mt-auto">
      ...
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-6 sm:px-6 lg:space-y-16 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div>
            {/* Logo */}
            <img src={Logo} alt="Logo" className="w-40 bg-none" />
              ...
              {socialLinks.map((social) => (
                <li key={social.name}>
                  <Link
                    to={social.path}
                    rel="noreferrer"
                    className="text-main transition hover:opacity-75"
                  >
            ...
            {footerLinks.map((column, index) => (
              <div key={index}>
                ...
                <ul className="space-y-2">
                  {/* Loop over the links for each column */}
                  {column.links.map((link, linkIndex) => (
                    <li key={linkIndex}>
                      {/* Link item */}
                      <Link
                        to={link.path}
                        className="text-sm text-gray-700 transition hover:opacity-75 hover:underline"
                      >
                        {link.label}
                      </Link>
                    </li>
