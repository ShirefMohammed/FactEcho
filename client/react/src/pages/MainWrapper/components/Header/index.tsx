    ...
    <header className="sticky top-0 z-999 flex w-full bg-white drop-shadow-1">
      <div className="w-full shadow-2">
        <div className="flex flex-grow items-center justify-between max-w-7xl mx-auto px-4 py-2  md:px-6 2xl:px-11">
          ...
          <NavLink to="/">
            <img src={Logo} alt="Logo" className="w-34 bg-none" />
          </NavLink>
...
const SearchForm: React.FC = () => {
  ...
  const navigate = useNavigate(); // Accessing the navigate function for routing
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault();
        if (searchKey) navigate(`/articles/search?searchKey=${searchKey}`); // Navigate to the search results page
      }}
    >
      <button
        type="submit"
