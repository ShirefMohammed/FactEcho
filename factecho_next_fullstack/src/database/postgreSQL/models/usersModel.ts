...
export class UsersModel implements UsersDao {
  ...
  private getUserSelectedFieldsWithAlias(
    ...
    if (!selectedFields || selectedFields.length === 0) {
      ...
      return this.userFields.map((field) => `${tableAlias}.${field}`).join(", ");
    }
    ...
    return selectedFieldsAfterFilter.map((field) => `${tableAlias}.${field}`).join(", ");
  }
  ...
  private getUserOAuthSelectedFieldsWithAlias(
    ...
    if (!selectedFields || selectedFields.length === 0) {
      ...
      return this.userOAuthFields.map((field) => `${tableAlias}.${field}`).join(", ");
    }
  ...
  async findUserById(
    ...
    try {
      ...
      const userFields = this.getUserSelectedFieldsWithAlias("u", selectedFields);
      const userOAuthFields = this.getUserOAuthSelectedFieldsWithAlias("uo", selectedFields);

      // Combine fields for the SELECT clause
      const selectFields = [userFields, userOAuthFields].filter(Boolean).join(", ");
   ...
   */
  async findUserAvatar(userId: IUser["user_id"]): Promise<IUserAvatar["avatar"] | null> {
    try {
  ...
  async getUsers(
    ...
    try {
      // Filter and prefix selected fields for `users` and `users_oauth` tables
      const userFields = this.getUserSelectedFieldsWithAlias("u", selectedFields);
      const oauthFields = this.getUserOAuthSelectedFieldsWithAlias("uo", selectedFields);
   ...
   */
  async updateUser(userId: IUser["user_id"], user: Partial<IUser>): Promise<Partial<IUser>> {
    try {
      ...
      const { rows } = await pool.query(query, [userId, ...Object.values(user)]);
  ...
  async setVerificationToken(
    ...
    try {
      if (verificationToken === "") {
        await pool.query(`DELETE FROM users_verification_tokens WHERE user_id = $1`, [userId]);
        return;
      }
  ...
  async setResetPasswordToken(
    ...
    try {
      if (resetPasswordToken === "") {
        await pool.query(`DELETE FROM users_reset_password_tokens WHERE user_id = $1`, [userId]);
        return;
      }
   ...
   * @returns void
   */
  async setAvatar(userId: IUser["user_id"], avatar: IUserAvatar["avatar"]): Promise<void> {
    try {
      if (avatar === "") {
        await pool.query(`DELETE FROM users_avatars WHERE user_id = $1`, [userId]);
        return;
      }
