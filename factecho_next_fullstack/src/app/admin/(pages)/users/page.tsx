...
import { ApiBodyResponse, GetUsersResponse, SearchUsersResponse } from "@/types/apiTypes";
...
const AdminUsers = () => {
  ...
  useEffect(() => {
    searchKey
      ? router.push(`/admin/users?searchKey=${searchKey}&page=${searchUsersPage}&limit=${limit}`)
      : router.push(`/admin/users?page=${usersPage}&limit=${limit}`);
  ...
  return (
    <tr className="border-b border-slate-300 dark:border-slate-700 text-sm text-body bg-white dark:bg-boxdark dark:text-bodydark font-normal">
      ...
      <td className="px-6 py-4 whitespace-nowrap flex items-center space-x-4">
        <Link
          href={`/users/${user.user_id}/overview`}
